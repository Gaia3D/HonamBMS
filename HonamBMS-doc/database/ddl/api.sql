...
comment on column api_log.server_id is '서버 고유키';
comment on column api_log.server_name is '서버명(중복)';
...
create table api_log_2032 (
	check ( insert_date >= to_timestamp('20320101000000000000', 'YYYYMMDDHH24MISSUS') and insert_date < to_timestamp('20330101000000000000', 'YYYYMMDDHH24MISSUS') )
) inherits (api_log);
create table api_log_2033 (
	check ( insert_date >= to_timestamp('20330101000000000000', 'YYYYMMDDHH24MISSUS') and insert_date < to_timestamp('20340101000000000000', 'YYYYMMDDHH24MISSUS') )
) inherits (api_log);
...
alter table only api_log_2032 add constraint api_log_2032_pk primary key (api_log_id);
alter table only api_log_2033 add constraint api_log_2033_pk primary key (api_log_id);
